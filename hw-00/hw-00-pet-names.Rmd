...
You should see that your Rmd (R Markdown) file and its output, your md file (Markdown), file are listed there as recently changed filed.
You should also see a file with the \`.Rproj\` extension, which is the RStudio project configuration file that got created with you cloned your repo into an RStudio project.
You don't need to worry so much about the contents of this file for now.
...
To do this, we can use the `top_n()` function.
...
✏️ *Now is a good time to commit and push your changes to GitHub with an appropriate commit message. Commit and push all changed files so that your Git pane is cleared up afterwards. Make sure that your last push to the repo comes before the deadline, 1 October, 16:00 UK time.*
